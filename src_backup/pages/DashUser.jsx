...
export default function DashUser() {
  ...
  const handleSubmit = () => {
    ...
    // formDataToSubmit.append('tanggalAwal', formData.tanggalAwal);
    // formDataToSubmit.append('tanggalAwal', formData.tanggalAwal);
    // formDataToSubmit.append('tanggalAkhir', formData.tanggalAkhir);
    ...
    //   formDataToSubmit.append('buktiFoto', formData.buktiFoto);
    // }
    ...
    //     },
    //   }
    //   const response = await fetchData('kehadiran', formDataToSubmit, headers, {
    //     "tanggal": getFormattedDate(),
    //     "wktdatang": getFormattedTime(),
    //     "wktpulang": null,
    //     "siswa": localStorage.getItem('id'),
    //     "status": checkStatus()
    //   }, 'post');
  ...
  React.useEffect(() => {
    const getData = async () => {
      try {
        ...
        var pulang = result.find(item => item.wktpulang === null) || false;
        var datafix = []
      ...
      <Box sx={{ display: 'flex' }}>
        ...
        >
          ...
          <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            {/* Buttons */}
            <Box sx={{ mb: 4 }}>
              <Button variant="contained" color="primary" sx={{ mr: 2 }} onClick={handlePresensi}>
